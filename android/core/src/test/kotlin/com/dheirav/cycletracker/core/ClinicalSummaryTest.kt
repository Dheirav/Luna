...
class ClinicalSummaryTest {
    ...
    /** Sections with nothing in them are omitted rather than printed empty — a clinician reading
     *  an empty heading has to work out whether it means "none" or "not recorded". */
    @Test
    fun `empty sections are omitted`() {
        val text = summary(CycleProjector.fromPeriods(periods("2024-01-01" to Source.OBSERVED)))

        assertFalse(text, text.contains("BLEEDING BETWEEN PERIODS"))
        assertFalse(text, text.contains("PATTERNS THE APP FLAGGED"))
        assertFalse(text, text.contains("SYMPTOMS BY PHASE"))
    }
